...
void Vector :: PrintVector( )const
{
    ...
    std::cout<<"]"<<std::endl; //for separating rows
}
void Vector :: SetVector(){
	std::cout<<"Enter the elemnts for Vector"<<std::endl;
	for(int i=0; i<mSize; i++) std::cin>>mData[i];
}

// Overloading square brackets
// Note that this uses â€˜zero-basedâ€™ indexing,
...
// Note that this uses â€˜one-basedâ€™ indexing,
// and a check on the validity of the index
double& Vector::operator()(int i)
{
	assert(i > 0);
